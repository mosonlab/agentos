...
| Node.js | Use `22.17.0`, recorded in `.nvmrc`. The top-level checks accept `^20.19.0 \|\| >=22.12.0`, but that broader range is not covered by every locked development dependency; Node 22.12.x and 23 can emit engine failures or warnings. |
...
Before the first runner start, add a stable, machine-unique identity to `.env`,
for example `RUNNER_ID=my-mac-agentos-1`. The generated file does not contain one;
without it the default is `<hostname>-<pid>`, which changes after every restart
and cannot reclaim workspaces left by the prior process.
...
does not mean PostgreSQL is ready to accept the release migration. If Compose
cannot bind the port, stop the existing service on 5432; do not point AgentOS at
an unknown PostgreSQL instance.
...
non-interactive write access separately against a disposable repository before
onboarding anything important. AgentOS sets `GIT_TERMINAL_PROMPT=0`; shell-only
authentication such as an inherited `SSH_AUTH_SOCK` is not passed into its
...
only that the Codex CLI exists and reports a signed-in session; it does not prove
model entitlement or full adapter-protocol compatibility. The deterministic
smoke task is the first end-to-end model-access check.
...
startup; it does not wait indefinitely for an absent API. If the API was not
ready and the runner exits, restart the runner.

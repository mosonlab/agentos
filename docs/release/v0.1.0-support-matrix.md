...
| Other Node.js versions accepted by `^20.19.0 \|\| >=22.12.0` | **Unverified** | The top-level checks accept them, but locked ESLint dependencies require `^20.19.0 \|\| ^22.13.0 \|\| >=24`; Node 22.12.x and 23 are therefore not a coherent verified range. |
...
| --- | --- | --- |
| Codex CLI | **Verified adapter; model access Pending smoke** | Startup preflight checks only the binary version command and login status. It does not prove entitlement to the starter model `gpt-5.6-sol:medium` or complete adapter compatibility; the deterministic smoke task is the first end-to-end check. |

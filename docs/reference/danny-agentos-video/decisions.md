...
- **单链定义**（2026-08-16 终版：废除轻/重链之分，一条链照抄原版九步；落地为一个种子 TaskTemplate，小任务由模板跳步解决，编排归模板不归阵容）：
  ① spec(fable:medium/CLAUDE)＋闸门 → ② plan(fable:medium/CLAUDE) → ③ 计划评审 review-coordinator(sol:high/CODEX，单会话覆盖可行性/范围/一致性/安全＋一次重点复查) → ④ plan-reviser(opus-5:high/CLAUDE) 按评审修订＋实施授权闸门 → ⑤ 路由选择实现（Critical = senior-dev opus-5:high/CLAUDE；Planned Standard = executioner opus-5:high/CLAUDE）→ ⑥ review-coordinator(sol:high/CODEX) 代码评审 → ⑦ senior-dev(opus-5:high/CLAUDE) 修复 → ⑧ librarian(luna:high/CODEX) 更新 wiki → ⑨ delegated exact-head human-role authorization。
  - **审批位置已由仓库治理覆盖**（2026-08-17）：原版视频在 ① spec 与 ② initial plan 放 Approval；AgentOS 当前以 `docs/governance/task-routing-v1.md` 为权威。Full Assurance 静态默认在 ① spec、④ 经评审修订后的 implementation-authority plan 和 ⑨ 人工 PR 审查设闸，② initial plan 不在评审前请示；短链按风险省略前两类闸门。
- **阵容裁决更新（2026-08-17，覆盖早期“收敛至 7”）**：规范源码阵容收敛至 9：default/frontend-dev/spec/plan/plan-reviser/review-coordinator/implementation-plan-executioner/senior-dev/librarian。`scope-guardian` 与 `coherence` 已删除；`feasibility` 与 `code-reviewer` 已从源码阵容移除，数据库历史记录只在 live/queued reference 清零后归档。四个评审 lens 与一次风险复查并入 `review-coordinator` 提示词（`agents/roles/review-coordinator.md` 已重写，不再散子代理，因此可跑 Sol/CODEX）。
...
- **统一评审角色（2026-08-17，取代前端链 `code-reviewer` 例外）：**③ 计划评审与 ⑥ 代码评审统一使用 `review-coordinator`。同一提示词接受 spec、plan 或 implementation diff，依次做 feasibility、scope、coherence、security 四个 lens，再做一次独立的高风险验证。模型或厂商不是独立性证据；所有结论从权威合同、准确 base/head、真实 diff 与实测重新推导。`feasibility` 与 `code-reviewer` 在 live/queued reference 清零后归档，不再进入新模板。`frontend-dev`/Opus High 只用于符合 Direct Standard 的 bounded frontend-only 路由，不强塞进九步链。

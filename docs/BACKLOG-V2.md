...
## 前端性能（看板修复期间发现，2026-08-16）

- [ ] **`GET /tasks` 112 条返回 1.36 MB（约 12 KB/行），每 2.5 秒轮询一次。**
      看板修复后客户端已经不再解析未变化的响应（先做文本比对再解析），
      但**网络传输与服务端序列化照跑**，实测约 300 条任务时会重新越过长任务门槛。
      修法：ETag 或 `max(updatedAt) + count` 短路——比分页的改动面小得多，且不动交互。
      现状实测（1440×900 视口）：看板高 697px、页面滚动范围 0、14 秒空闲 0 次长任务。

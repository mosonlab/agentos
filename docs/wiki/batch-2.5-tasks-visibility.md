...
manual release, the five-column board, task archiving, recurring automations,
and webhook triggers. The main implementation is in
...
[`packages/api/src/scheduler.ts`](../../packages/api/src/scheduler.ts), and
the pages and pure modules under [`apps/web/src`](../../apps/web/src).
...
Tasks also carry three pieces of provenance/lifecycle state:
...
`webhookReplayWindowSec`, and the append-only `TriggerFire` ledger. The schema
...
unarchived, is `TODO` or `BACKLOG`, has no active run, and is below its run
budget. For the new task-visibility guards, an active run means
`QUEUED`, `CLAIMED`, `PROVISIONING`, `RUNNING`, or `WAITING_INBOX`; a waiting
Inbox decision is still live work.

`POST /tasks/:taskId/start` takes the Task-row lock, rechecks those facts, and
creates the next run in the same transaction. A successful Backlog start
changes the task to `TODO`. A stale or invalid click receives the specific
`409`/`400` reason instead of creating an out-of-order run or leaking a server
error. Double-presses are serialized by the row lock and leave one run.
...
successor can later be released with `Start now`; a successor with an archived
assignee is parked in `REVIEW` with a failure reason. Human successors reached
...
and archive reasons beneath the step name, and never recomputes the API's
startability decision in the browser.
...
API-provided chain marker (`n/m · active step · task status`), a `cron` pill
for CRON copies, and a `webhook` pill for webhook-created tasks. Manual tasks
have no source pill. Same-column drops are ignored; all other drops are status
PATCHes and do not enqueue a run.
...
- Chain progress is intentionally API-owned. The web marker only formats the
  API's values; adding a second arithmetic implementation risks disagreeing
  with archived-row and sparse-index semantics.
...
- A successor parked in REVIEW because its assignee is archived has no dedicated
  release button yet; unarchive the agent and correct the task status manually.
...
scoping, parked successors, start/archive races, archive-vs-scheduler races,
replay-window behavior, paused triggers, manual fire ledger atomicity, and
source backfill. The web suites cover the chain marker/card, five board
...
the board, Archived, Automations, Triggers, trigger detail, task detail, and
both themes.

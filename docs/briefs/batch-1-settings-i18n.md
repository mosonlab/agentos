...
- ⑤ implementation and ⑦ fixes: `frontend-dev` (claude-opus-5:xhigh / CLAUDE). ⑥ code review: review-coordinator (Sol / CODEX) — cross-vendor rule: Opus writes, Sol reviews.
...
## Standing clauses
- Field name for task creation is `name`, not `title`. Implementation steps set `maxDurationMin: 240`. Any position formerly using Fable uses `claude-opus-5:xhigh`. Never write OPERATOR_TOKEN into any artifact.

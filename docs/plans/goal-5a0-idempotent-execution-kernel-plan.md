...
Product Contract: Goal 5a0 v1.1 (bumped in round 6; see "Product Contract v1.1 decision" below)
...
All six rounds stay in the append-only disposition ledger below. Where a later round contradicts an earlier one, the later round governs and the earlier entry remains visible as history.
...
## Product Contract v1.1 decision
...
**Product-owner authorization of record.** Leo's direction carried in AgentOS task `cmsxuitsw04ahmpmxsqxea2jq` ("Revise PR #94 typed authorization and durable handoff"): *Leo explicitly requires fresh exact-head delegated authorization with exact SHA/base/check/review evidence and has authorized this continuing goal*, together with that task's standing instruction to bump the Product Contract to v1.1 in the artifacts and record the one-time authorization evidence if the closure materially changes scope, authority, or required evidence. That is the one-time product-owner decision evidence for v1.1. No separate approval is invented here, and no Inbox question duplicates it.

**What v1.1 does not change.** The objective; the Goal 5a0 kernel's in-scope and out-of-scope work; the kernel's own acceptance criteria; gates 1-6; the Planned Critical route and the future `senior-dev`/high-effort implementation role; the route-specific 400/409 classification; the canonical row order; the cancellation fence and its non-scope list; the directory-only migration-tail procedure; round 4's identifier discipline; and the prohibition on production migration, restart, enablement, public release, and merging anything. v1.1 changes only the merge-authorization and dependency-handoff **evidence and authority mechanics**: gate 7, "Implementation identity and durable handoff record", "Dependency-gate dry checks", and their tests in Step 15. Dependency D1 is explicitly **not** Goal 5a0 implementation scope, and Goal 5a0 does not implement it.
...
7. **Inbox 3a fuse.** Goal 5a0 owns and may change the symbols listed in the ownership table below; it must not implement Inbox 3a. Inbox 3a (issue #98) does not begin until every one of the following holds: the **actual Goal 5a0 implementation pull request** — the one recorded under "Implementation identity and durable handoff record", never issue #97 itself — has merged with a two-parent merge commit; the implementation task that produced it was found through the **external locator** on issue #97 rather than through any document inside that pull request; exactly one live `goal5a0.merge_handoff` record resolves on that task under the total order and supersession algebra defined below; that handoff names the exact activity ID of a live `goal5a0.merge_authorization` record and carries a byte-identical copy of its authorized head, base, required-check conclusions, and evidence digest; the authorization is corroborated by its GitHub-server-bound half — an `APPROVED` review by an allowlisted login whose `commit_id` equals the authorized head and whose `submitted_at` precedes `mergedAt`; and the recorded merge commit is proven an ancestor of a freshly fetched `origin/master`. The five dry checks in "Dependency-gate dry checks" below decide `STOPPED_FOR_REROUTE` versus `SAFE_TO_IMPLEMENT`; there is no third outcome and no partial credit. Closing issue #97 is not evidence of a merge; neither is an open or unmerged linked pull request, a document that names a merge commit, an AgentOS activity whose authorization half is missing or stale, or a green CI badge. An ambiguous, absent, duplicated, superseded, malformed, or drifted record is `STOPPED_FOR_REROUTE`, a stop condition rather than a follow-up. Goal 5a0 leaves a written handoff, not a partial Inbox implementation.

8. **Delegated-identity fuse.** No step of this plan may claim that AgentOS attributes a write to a named human. `packages/api/src/auth.ts:18-45` authenticates one shared `OPERATOR_TOKEN` into an anonymous `{ kind: "operator" }` principal, and `POST /tasks/:taskId/activity` (`packages/api/src/app.ts:2717-2728`) hardcodes `actorType: "operator"` while copying `actorId` and `metadata` verbatim from the client. Any authorization evidence therefore fails closed unless its GitHub-server-bound half is present and agrees; a stage-2 record alone is a stop. Implementing server-derived operator identity is **dependency D1**, defined below, and is outside Goal 5a0's implementation scope.
...
6. **Record the implementation identity, stage by stage, as typed records, and assign each stage a producer.** The three stages of "Implementation identity and durable handoff record" have three different owners, and only the first belongs to this plan's implementation Agent Run. Every record is one `TaskActivity` on issue #97's implementation task whose `metadata` carries the envelope `{ kind, schemaVersion: 1, issueNumber: 97, implementationTaskId, supersedesActivityId }` plus that kind's payload.
   ...
   2. **Stage 2, the delegated exact-head authorizer.** Before the merge, that independent operator writes one `goal5a0.merge_authorization` record on the same task through `POST /tasks/:taskId/activity`, carrying `prNumber`, `prUrl`, `authorizedHead`, `authorizedBase`, `requiredChecks` (sorted by name, each `headSha == authorizedHead`), `review` (`reviewId`, `reviewerLogin`, `reviewState`, `reviewCommitId`, `submittedAt`), `evidenceDigest`, and `authorizerLogin`, and re-writes it as a new record superseding the old on any head, base, check, or material-evidence drift. The authorization is only evidence when its **GitHub-server-bound half** exists: an `APPROVED` review by an allowlisted login whose `commit_id` equals `authorizedHead` and whose `submitted_at` precedes `mergedAt`. The AgentOS record alone is never sufficient, because `packages/api/src/auth.ts:18-45` authenticates one shared token into an anonymous principal and `app.ts:2717-2728` copies `actorId` verbatim from the client.
   3. **Stage 3, the delegated operator at merge closeout.** After an exact-head-match merge, the same operator writes exactly one `goal5a0.merge_handoff` record naming `governingAuthorizationActivityId` — the exact `TaskActivity.id` of the live stage-2 record — and copying that record's `authorizedHead`, `authorizedBase`, `requiredChecks`, and `evidenceDigest` byte-for-byte, plus `mergeCommit`, `mergeParents` (exactly two, from `gh api repos/mosonlab/agentos/commits/<mergeCommit>`), `mergedAt`, `masterSha`, and `ancestryVerified`, having first run `git fetch origin master` and `git merge-base --is-ancestor <mergeCommit> origin/master`. **This step is not assigned to the implementation Agent Run**, which has already completed, and it is not a file: it is the single durable handoff issue #98 consumes. A pull request closed unmerged or superseded gets a `goal5a0.merge_invalidated` record naming the exact activity IDs it kills, and the successor pull request gets a **new** stage-1 record written by its own Run or by the operator — no completed Run is ever asked to rewrite a task output.
...
7. **Make the gate executable and prove it.** Author the dependency-gate harness, its classifiers, and its tests, and run them as part of this step's evidence:
   ...
   1. `scripts/goal-5a0-dependency-gate.sh` implements the artifact-hygiene harness and dry checks L, 0, A, C, and B exactly as written in "Dependency-gate dry checks": one validated `mktemp -d` root, the `record` helper writing `<label>.out`/`<label>.err` and appending to `exit-status.tsv`, the evidence copy-out, and the `trap`-based cleanup that fires only after the copy. It prints exactly one of `STOPPED_FOR_REROUTE` or `SAFE_TO_IMPLEMENT` and writes nothing to the repository or the working tree.
   2. `scripts/goal-5a0-dependency-gate.test.mjs` (run by a root `test:dependency-gate` script, alongside the existing `test:snapshot-scan`) drives the classifiers over the recorded fixtures: absent, open, closed-unmerged, merged, `mergeCommit: {}`, single-parent merge commit, and one-field-different handoff records, asserting the exact outcome token for each. It also runs the resolution algorithm over the ten record fixtures — duplicate at equal `createdAt`, corrected at equal `createdAt`, conflicting, invalidation after handoff, invalidation then successor, invalidation naming an absent record, empty list, `schemaVersion: 2`, foreign `implementationTaskId`, and an unrelated non-Goal activity — each asserted **in both input orders** to prove the `(createdAt, id)` total order makes the outcome order-independent.
   3. `packages/api/src/goal-5a0-handoff.dbtest.ts` proves the same behaviour against real PostgreSQL through the real routes: two records written with identical `createdAt` come back from `GET /tasks/:taskId/activity` in an order the endpoint does not fix, and the resolver still returns one outcome; a duplicate stops; a conflicting pair stops; a corrected pair resolves to the superseder; an invalidation written before its target stops as a dangling reference and one written after supersedes; a successor pull request after an invalidation resolves to the successor's records with no field carried forward from the abandoned one; `DELETE /tasks/:taskId` leaves `GET /tasks/:taskId/activity` answering `200 []` while `GET /tasks/:taskId` answers 404, so the check reports `HANDOFF_TASK_MISSING` rather than "no handoff yet"; a record whose `implementationTaskId` names another task is rejected; and a request that sets `actorType` in the body still lands as `"operator"`, proving `actorType` cannot be forged while `actorId` can.
   4. The negative authorization suite asserts `STOPPED_FOR_REROUTE` for: a handoff whose `governingAuthorizationActivityId` names nothing, names a record on another task, or names a superseded authorization; a stage-2 record whose activity `createdAt` is not earlier than `mergedAt`; `headRefOid` or `mergeParents[1]` differing from `authorizedHead`; `mergeParents[0]` differing from `authorizedBase`; a single-parent merge commit; a required check whose conclusion changed or disappeared, or whose `head_sha` is not `authorizedHead`; an `evidenceDigest` that does not recompute from the copied tuple; a later `goal5a0.implementation_identity` whose `implHead` differs from `authorizedHead`; and a forged `authorizerLogin` or `actorId` with no matching allowlisted `APPROVED` GitHub review at that commit.

**Verification:** Every reviewer checklist answer is backed by a file/command/test/result in the evidence document. The final invariant query returns zero rows. `git diff --check` is clean, the reviewed SHA matches the evidence SHA, and the disposition ledger contains every finding from every review round. Issue #97's implementation task carries exactly one live `goal5a0.implementation_identity` record whose three fields are each a real value or their literal placeholder and none an invented or predicted pull-request number, plus the non-authoritative task-output mirror; the evidence document and the runbook carry the stage-2/stage-3 procedure, producers, allowlist, and payload schema with no post-merge value asserted and no requirement to rewrite them after the merge; the external locator block on issue #97 resolves to that task ID and is corroborated by `GET /tasks/<taskId>` and a single-match list query; and the merge commit appears only in the stage-3 record written after this task is complete, validated by dry check 0 on retrieval, bound to its authorization by activity ID and byte-identical copy with a recomputing `evidenceDigest`, corroborated by an allowlisted `APPROVED` GitHub review at the authorized head, and re-proved by `git merge-base --is-ancestor <mergeCommit> origin/master` after `git fetch origin master`. Dry check A's `gh pr view --json` field list is verified against `gh pr view --json` with no value on the tree's installed `gh`, and the reviews and check-runs element keys are printed with `jq '.[0]|keys'` and `jq '.check_runs[0]|keys'`, before the evidence packet is closed. `npm run test:dependency-gate` and the handoff dbtest both pass, every classifier and resolver fixture producing its stated token in both input orders; the harness transcript in the packet shows one `mktemp -d` root, a complete `exit-status.tsv`, the copied-out evidence, and the removed temporary root, with no dry-check artifact written into the repository or the working tree.
...
2. When that pull request is merged, the delegated operator runs the five dependency-gate dry checks below — locate the implementation task through the external locator on issue #97, resolve the durable handoff record, confirm GitHub still agrees with it, confirm the authorization is server-corroborated and undrifted, confirm ancestry — and hands issue #98 their raw transcripts; checks L and 0 need operator authentication, which an Agent Run does not have. On `SAFE_TO_IMPLEMENT`, issue #98's first action is to rerun this plan's Step 1 revalidation against the merged tail, recording the merged Goal migration folder name and the merged state of every symbol in the ownership table.
...
**Stage 2 — `goal5a0.merge_authorization`, before the merge.** This is the one-time delegated exact-head authorization required by `docs/governance/task-routing-v1.md` §"Human approval placement". Payload beyond the envelope:

| Key | Value |
| --- | --- |
| `prNumber`, `prUrl` | the pull request being authorized |
| `authorizedHead` | the exact 40-hex head that was reviewed |
| `authorizedBase` | the exact 40-hex base commit it was reviewed against |
| `requiredChecks` | array, sorted by `name`, of `{ name, conclusion, headSha }` read from `gh api repos/mosonlab/agentos/commits/<authorizedHead>/check-runs`; every element's `headSha` must equal `authorizedHead` |
| `review` | `{ reviewId, reviewerLogin, reviewState, reviewCommitId, submittedAt }` copied from the GitHub review object that carries this authorization |
| `evidenceDigest` | the 64-hex digest defined below over exactly the five keys above |
| `authorizerLogin` | the GitHub login of the authorizing human; a **claim**, verified only against `review.reviewerLogin` and GitHub itself |

The **evidence digest** is computed from a canonical JSON document containing exactly `{prNumber, prUrl, authorizedHead, authorizedBase, requiredChecks, review}` and nothing else:

```sh
printf '%s' "$EVIDENCE_JSON" | jq -S -c . | shasum -a 256 | cut -d' ' -f1
```

`jq -S -c` sorts keys at every level and removes insignificant whitespace, so the digest is independent of key order and formatting; the pipeline including its trailing newline is part of the definition and must not be varied. Verified during this revision on `jq` 1.7.1: two orderings of the same object produced the identical digest `b3ebc93a638763db7b4aa3dd2048802d09aa45c115fc64b44165229da755ac07`. On Linux, `sha256sum` replaces `shasum -a 256`; the runbook records which was used.

Head, base, required-check, or material-evidence drift invalidates a stage-2 record. Re-authorization is a **new** record whose `supersedesActivityId` names the old one. The authorization authorizes the merge only; it is neither implementation authorization nor the handoff.

**Stage 3 — `goal5a0.merge_handoff`, after the merge.** The single durable handoff issue #98 consumes. Payload beyond the envelope:

| Key | Value |
| --- | --- |
| `prNumber`, `prUrl` | from the live stage-1 record |
...
| `authorizedHead`, `authorizedBase`, `requiredChecks`, `evidenceDigest` | **copied byte-for-byte** from that authorization record |
...
Reference **and** copy are both required, and this is the point of the design: the reference proves *which* authorization governed the merge, while the copy makes drift detectable in one fetch and makes tampering self-evident, because a change to either copy breaks equality with the other and breaks the digest recomputed from the copied tuple. The merge may proceed only on an exact-head match — the head actually merged must equal the governing `authorizedHead` — and `mergeParents[1]` is the executable proof of that match.

**`goal5a0.merge_invalidated`.** Payload beyond the envelope: `invalidatesActivityId` (the exact `TaskActivity.id` being killed, which must already exist and must precede this record in the total order), `abandonedPrNumber`, and `reason`. It is the only way to retract a record, because no route can edit or delete one.

#### External, non-circular locator

Issue #98 must find issue #97's implementation task **without** reading anything inside the pull request it is gating on, or the locator would be circular. The locator of record is a fenced block in the body of GitHub issue #97, or in a comment on it, written by an allowlisted login:

```text
agentos-implementation-task: <40-char AgentOS task ID>
agentos-api-base: https://<host>
```

It is non-circular because issue #97 exists before the implementation pull request, is outside the merged tree, and names the task; the task's activities then name the pull request. It is retrieved with `gh issue view 97 --json number,state,title,url,body,author,comments` — all supported fields on `gh` 2.89.0 — and GitHub authenticates the author of the body and of every comment, so the locator's writer is server-identified even though the AgentOS writer is not.

The locator is corroborated, never trusted alone: `GET /tasks/<taskId>` must return 200 with that exact ID, and `GET /tasks?projectId=<recorded project>&archived=all` must contain exactly one task whose `name` equals the recorded implementation-task name. Zero matches, more than one match, a disagreement between the two sides, a locator written by a login outside the allowlist, more than one locator block naming different task IDs, or a 404 from `GET /tasks/<taskId>` are each `STOPPED_FOR_REROUTE`. The allowlist of authorized logins lives in the runbook as a governance artifact; it is not supplied at check time by whoever is running the check.

#### Total order and resolution

`GET /tasks/:taskId/activity` orders only by `createdAt`, and `createdAt` is a PostgreSQL `timestamp(3)`, so two records written in the same millisecond come back in an order the database is free to vary between calls. The consumer therefore re-sorts client-side into the total order **`(createdAt ASC, id ASC)`** — deterministic by construction, since `TaskActivity.id` is unique — and resolves each kind with this algorithm:

1. Keep only activities whose `metadata` is an object with a `kind` in the four literals. Everything else on the task — "Task created", runner completion notes, ordinary agent progress — is ignored.
2. For every kept record: `schemaVersion` must be `1` and `implementationTaskId` must equal the task being read. Otherwise stop.
3. Sort into `(createdAt ASC, id ASC)`.
4. Resolve references: `supersedesActivityId` and `invalidatesActivityId` must each name a kept record that appears **earlier** in the total order. A self-reference, a dangling reference, or a forward reference is a stop.
5. A record is dead if any later record names it in either reference field. The live set for a kind is the kept records of that kind that are not dead.
6. Exactly one live record is required for `goal5a0.implementation_identity`, `goal5a0.merge_authorization`, and `goal5a0.merge_handoff`. Zero is a stop; two or more — duplicate, conflicting, or merely repeated — is a stop. There is no "latest wins" rule, because "latest" is exactly what an equal-millisecond tie cannot decide.

Step 4 is what makes ordering safe rather than merely deterministic: a correction must arrive *after* what it corrects, so a tie between a record and its own superseder is impossible, and a tie between two unrelated records of the same kind resolves to "two live records", which stops. Executed during this revision against ten fixtures, every one order-independent — duplicate at equal timestamps stops; corrected-at-equal-timestamps resolves to the superseder in both input orders; conflicting stops; invalidation after a handoff stops; invalidation followed by a successor handoff resolves to the successor; invalidation naming an absent record stops as a dangling reference; an empty list stops; `schemaVersion: 2` stops; a foreign `implementationTaskId` stops; and an unrelated non-Goal activity is ignored without affecting the result.
...
3. A fresh stage-2 authorization is required for the successor head; no reviewed head, check conclusion, evidence digest, merge commit, or handoff is carried forward.
...
4. `GET /tasks/:taskId/activity` ordering by `[{ createdAt: "asc" }, { id: "asc" }]` and accepting a `kind` filter, so the total order is server-side rather than reconstructed by every consumer.

D1 is **not** Goal 5a0 implementation scope, is not scheduled by this plan, and is deliberately not given an invented issue number. Until it exists, the GitHub-server-bound half of the authorization is mandatory, not optional — and it stays mandatory afterwards, because two independent authorities are stronger than one.

#### Residual risks accepted at v1.1

| Risk | Why it is accepted | What detects it |
| --- | --- | --- |
| Any `OPERATOR_TOKEN` holder can write a stage-2 or stage-3 record claiming any `authorizerLogin` | AgentOS has no per-human identity until D1 | The GitHub review half: an `APPROVED` review by an allowlisted login at `commit_id == authorizedHead` cannot be forged with an AgentOS token; a claim without it stops the gate |
| Any `OPERATOR_TOKEN` holder can delete the implementation task and its whole handoff | Adding a delete guard is out of scope for this contract | `GET /tasks/<taskId>` 404 → `HANDOFF_TASK_MISSING`; never reported as "not yet merged" |
| Spurious extra records can be appended at any time | The log is append-only by design and records cannot be edited | Two live records of a singleton kind stop the gate; a correction requires an explicit supersession reference |
| The AgentOS clock and the GitHub clock are independent | Cross-service ordering has no shared clock | Ordering is also proved clock-free through `mergeParents[1] == authorizedHead` and `review.commit_id == authorizedHead`; the timestamp comparison is an additional stop, never the only one |
...
**Artifact hygiene — one validated temporary root, recorded exit statuses, cleanup after capture.** Every artifact of every check is written under a single `mktemp -d` root; nothing is written to the repository, the working tree, or a fixed path such as `pr-view.json` in the current directory. The root is validated before use, every command's exit status is recorded, and the root is removed only after the evidence has been copied out:

```sh
#!/bin/sh
set -o pipefail
GATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/goal5a0-gate.XXXXXXXX")" || { echo STOPPED_FOR_REROUTE; exit 1; }
case "$GATE_DIR" in
...
EVIDENCE_DIR="${1:?evidence dir required}"; mkdir -p "$EVIDENCE_DIR"
trap 'rm -rf "$GATE_DIR"' EXIT INT TERM
STATUS_LOG="$GATE_DIR/exit-status.tsv"; printf 'label\texit\n' > "$STATUS_LOG"
record() {
  label="$1"; shift
  "$@" > "$GATE_DIR/$label.out" 2> "$GATE_DIR/$label.err"; rc=$?
  printf '%s\t%d\n' "$label" "$rc" >> "$STATUS_LOG"
  return "$rc"
}
# ... checks L, 0, A, C, B, each invoked through `record` ...
cp -R "$GATE_DIR/." "$EVIDENCE_DIR/" || { echo "STOPPED_FOR_REROUTE evidence capture failed"; exit 1; }
```

The `trap` fires after the copy, so the temporary root never survives the run and the evidence always does. Executed during this revision on `sh`/`jq` 1.7.1/`git` 2.48.1: the five classifier fixtures and two `git merge-base` probes produced the exit-status table below, the transcript and fixtures were copied into the evidence directory, and the temporary root was gone afterwards (`ls` on it returned "No such file or directory").
...
Read issue #97 with `gh issue view 97 --json number,state,title,url,body,author,comments`, extract every `agentos-implementation-task:` block from the body and the comments, and keep only those written by an allowlisted login. Then corroborate against AgentOS with `GET /tasks/<taskId>` and `GET /tasks?projectId=<recorded project>&archived=all`. `STOPPED_FOR_REROUTE` when: no locator block exists; two blocks name different task IDs; the block's author is not allowlisted; the ID is not a valid AgentOS task ID; `GET /tasks/<taskId>` returns 404 (diagnostic `HANDOFF_TASK_MISSING`); the list query returns zero or more than one task with the recorded name; or the two sides disagree. Nothing inside the implementation pull request is read by this check.

**Dry check 0 — retrieve and resolve the durable handoff record.**

`GET /tasks/<taskId>/activity` with operator authentication, then apply the six-step resolution algorithm above to `goal5a0.merge_handoff`, `goal5a0.merge_authorization`, and `goal5a0.implementation_identity`. `STOPPED_FOR_REROUTE` when any of:
...
- **superseded** — the only candidate is named by a later `goal5a0.merge_invalidated` or by a later record's `supersedesActivityId`;
- **malformed** — a known `kind` with `schemaVersion != 1`; a missing, empty, or placeholder payload key; `mergeCommit`, `authorizedHead`, `authorizedBase`, or a `mergeParents` element failing `^[0-9a-f]{40}$`; `ancestryVerified != "ANCESTOR"`; `issueNumber != 97`; `implementationTaskId` disagreeing with the task read; `mergeParents` whose length is not exactly 2;
- **unresolvable reference** — `supersedesActivityId` or `invalidatesActivityId` naming a record that is absent, is the record itself, or appears later in the total order;
- **unbound** — `governingAuthorizationActivityId` naming no live `goal5a0.merge_authorization` on this task, or naming one whose `authorizedHead`, `authorizedBase`, `requiredChecks`, or `evidenceDigest` is not byte-identical to the handoff's copy, or whose digest recomputed from the copied tuple does not equal `evidenceDigest`.
...
jq -er --arg head "$HANDOFF_AUTHORIZED_HEAD" --arg base "$HANDOFF_AUTHORIZED_BASE" '
  if ((.parents | length) != 2)      then "STOPPED_FOR_REROUTE"   # squash or rebase merge
...
**Merged success is exactly this conjunction:** `state == "MERGED"`; `mergedAt` non-null; `mergeCommit.oid` matching `^[0-9a-f]{40}$`; `number`/`url`/`headRefOid`/`mergeCommit.oid`/`mergedAt` all equal to the handoff record's `prNumber`/`prUrl`/`authorizedHead`/`mergeCommit`/`mergedAt`; and the merge commit having exactly two parents equal to the handoff's `authorizedBase` and `authorizedHead` in that order. A `headRefOid` that no longer equals `authorizedHead` is head drift: the merge did not happen at the authorized head, so it is a disagreement and a stop, not a warning. A single-parent merge commit — a squash or rebase merge — is a stop, because the head-to-merge binding it would need is not recoverable from the commit graph. Everything else — nonexistent pull request, CLI failure, JSON parse or schema error, missing field, `state: "OPEN"`, `state: "CLOSED"` with a null `mergedAt`, or any field disagreement — is `STOPPED_FOR_REROUTE`.
...
Check 0 has already proved that the handoff names a live `goal5a0.merge_authorization` and copies it byte-for-byte with a digest that recomputes. Check C proves the authorization is *real, identified, and current* using the half of the evidence GitHub authenticates:

```sh
record reviews     gh api "repos/mosonlab/agentos/pulls/$HANDOFF_PR_NUMBER/reviews"
record check-runs  gh api "repos/mosonlab/agentos/commits/$HANDOFF_AUTHORIZED_HEAD/check-runs"
```

The review object's shape was confirmed live during this revision on `gh` 2.89.0 against a public pull request: each element carries `id`, `user.login`, `state`, `commit_id`, `submitted_at`, `author_association`, `body`, `html_url`, `node_id`, `pull_request_url`, `_links`. The same standing rule as the `gh pr view` field list applies to both endpoints — before trusting either classifier, print `.[0] | keys` (reviews) and `.check_runs[0] | keys` (check runs) on the tree's installed `gh` and use only names that appear.

`SAFE_TO_IMPLEMENT` requires all of:

- A review element with `state == "APPROVED"`, `user.login` in the runbook's allowlist, `commit_id == authorizedHead`, and `submitted_at` strictly earlier than the pull request's `mergedAt`. This is the server-bound authorization half; it is what proves an identified human authorized *this exact head* before the merge.
- `review.reviewerLogin`, `review.reviewId`, `review.reviewCommitId`, `review.reviewState`, and `review.submittedAt` in the stage-2 record all equal that GitHub review's fields, and the record's `authorizerLogin` equal to its `user.login`. A forged `actorId` or `authorizerLogin` fails here, because GitHub will not have a matching review.
- The stage-2 activity's server-assigned `createdAt` strictly earlier than `mergedAt`. `createdAt` is `@default(now())` and no route accepts a client-supplied value (`schema.prisma:568`), so this ordering cannot be back-dated from the client. It is an additional stop, never the only ordering proof: `mergeParents[1] == authorizedHead` and `review.commit_id == authorizedHead` prove the binding without any clock.
- Every element of `requiredChecks` present in the live check-runs response for `authorizedHead` with the same `name` and the same `conclusion`, and every check-run element's `head_sha == authorizedHead`. A conclusion that changed, a required check that disappeared, or an added failing required check is check drift and a stop.
- No live `goal5a0.merge_authorization` other than the one the handoff names, and no `goal5a0.implementation_identity` record whose `implHead` differs from `authorizedHead` appearing later in the total order than the authorization — either is stale delegation: the head moved after authorization, so the authorization no longer governs.

Any missing element, any disagreement, an unparseable response, or a non-zero `gh` exit is `STOPPED_FOR_REROUTE`.
...
| Deterministic `(createdAt, id)` total order and supersession algebra — equal-millisecond ties, duplicate, conflicting, corrected, superseded, dangling, forward, and foreign-task records all resolved or stopped identically in both input orders | 14, 15 |
...
| Typed stage-2 authorization bound into stage 3 by exact activity ID plus byte-identical copy of head, base, required-check conclusions, and evidence digest, proved to precede the merge and to have remained current | 14, 15 |
| Fail-closed server-bound delegated-principal evidence — the GitHub `APPROVED`-review half at the authorized head by an allowlisted login, the explicit statement that the shared `OPERATOR_TOKEN` and client `actorId` are not server-derived identity, and dependency D1 named but not implemented | 14, 15 |
| Executable dependency gate — five dry checks, only `gh`-supported JSON fields, `baseRefOid` rejected as base-at-merge evidence, null-safe `mergeCommit.oid`, two-parent merge-commit binding, explicit merged-success conjunction, every failure mode mapped to `STOPPED_FOR_REROUTE`, recorded fixtures | 14, 15 |
...
| Dry-check artifact hygiene — one validated `mktemp -d` root, recorded exit statuses, evidence copied out before `trap` cleanup, nothing written to the repository | 14, 15 |
...
Inbox 3a (issue #98) stays dependency-held until the actual Goal 5a0 implementation pull request — recorded under "Implementation identity and durable handoff record", never issue #97 itself — has merged with a two-parent merge commit, dry check L locates issue #97's implementation task through the external locator on the issue rather than through any document inside that pull request, dry check 0 resolves exactly one live `goal5a0.merge_handoff` record bound to a live `goal5a0.merge_authorization` whose evidence digest recomputes, dry check A finds GitHub agreeing with it field for field and the merge commit's two parents equal to the authorized base and head, dry check C finds the GitHub-server-bound authorization half — an `APPROVED` review by an allowlisted login at the authorized head, submitted before `mergedAt`, with undrifted required checks — and dry check B exits `0` against a freshly fetched `origin/master`, and Step 1's revalidation is rerun on that tail. Goal 5a0 discharges that boundary with the Step 9 fence and seam, the typed three-stage identity lifecycle with its named producers, the executable gate, and the written handoff; it does not implement Inbox 3a, it does not implement dependency D1, and it does not write the post-merge record itself — the delegated operator does, at merge closeout.

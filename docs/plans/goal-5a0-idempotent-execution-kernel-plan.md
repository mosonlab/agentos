...
All five rounds stay in the append-only disposition ledger below. Where a later round contradicts an earlier one, the later round governs and the earlier entry remains visible as history.
...
7. **Inbox 3a fuse.** Goal 5a0 owns and may change the symbols listed in the ownership table below; it must not implement Inbox 3a. Inbox 3a (issue #98) does not begin until the **actual Goal 5a0 implementation pull request** — the one recorded under "Implementation identity and durable handoff record", not issue #97 itself — is merged, that merge is recorded in one valid `implementation_merge_handoff` TaskActivity on issue #97's implementation task, and its recorded merge commit is proven an ancestor of a freshly fetched `origin/master` before Step 1's revalidation is rerun on that merged tail. Closing issue #97 is not evidence of a merge, and neither is an open or unmerged linked pull request, nor any document that names a merge commit. The three dry checks in "Dependency-gate dry checks" below decide `STOPPED_FOR_REROUTE` versus `SAFE_TO_IMPLEMENT`; an ambiguous or absent record is `STOPPED_FOR_REROUTE`, a stop condition rather than a follow-up. Goal 5a0 leaves a written handoff, not a partial Inbox implementation.
...
5. Add an "Inbox 3a handoff" section reproducing the dependency decision, the three lifecycle stages and producers from "Implementation identity and durable handoff record" — including the `implementation_merge_handoff` payload schema, the `implementation_merge_invalidated` reset rule, and the statement that the runbook itself is a **procedure and schema document, not a record**: it keeps the stage-1 placeholders, never claims to hold the merge commit, and is not rewritten after the merge — and all three dependency-gate dry checks verbatim (dry check 0's handoff retrieval and its missing/duplicate/conflicting/superseded/mismatched rejections; dry check A's exact supported `gh pr view --json number,state,mergedAt,mergeCommit,headRefOid,url` field list, the two `jq` classifiers, the merged-success conjunction, and the four dry fixtures with their required outcome tokens; dry check B's `git fetch` + `git merge-base --is-ancestor` commands), with the `STOPPED_FOR_REROUTE`/`SAFE_TO_IMPLEMENT` outcomes, the ownership inventory, the directory-only migration-tail rule from Step 1.6, the "Canonical Goal execution row order" sentence from Step 4.9 with the note that PR #95's two conflicting orders must be replaced by it and that #95 stays dependency-held until it also adopts the actual-PR identity rule, and the `goalRunResumeAuthority` seam, so the Inbox 3a work stream reads one operational document rather than reconstructing the boundary from this plan's steps. State in that section that issues #97 and #98 are work tickets that close and never merge, that the post-merge facts live in the AgentOS TaskActivity written by the delegated operator at merge closeout and nowhere else, and that no field may hold an invented number. Record the runbook's own `gh --version` at authoring time next to the field list, so a later reader can tell whether the supported-field set must be rechecked. State plainly what an operator sees: cancelling a Goal with a waiting Inbox card **closes the card, terminalizes and fences the Run, and does not resume**; an answer that arrives afterwards is a harmless duplicate that records nothing — no reply, no decision row — while an answer that arrived *before* the cancel keeps its recorded reply and decision row, which the cancel does not erase.
...
6. **Record the implementation identity, stage by stage, and assign each stage a producer.** The three stages of "Implementation identity and durable handoff record" have three different owners, and only the first belongs to this plan's implementation Agent Run:

   1. **Stage 1, this step's own work.** Issue #97's implementation Agent Run writes `ACTUAL_IMPL_PR_NUMBER`, `ACTUAL_IMPL_PR_URL`, and `ACTUAL_IMPL_HEAD` into the AgentOS task output of its own implementation task, using the placeholders until each value exists. It writes no authorization and no merge commit; both are unknowable while it runs.
   2. **Stage 2, the delegated exact-head authorizer.** Before the merge, that independent operator writes the exact-head authorization TaskActivity on the same implementation task — pull request, reviewed head, base, required-check conclusions, review evidence — and re-writes it as a new activity on any head, base, check, or material-evidence drift.
   3. **Stage 3, the delegated operator at merge closeout.** After the merge, the same operator writes exactly one `implementation_merge_handoff` TaskActivity with the full stage-3 payload, having first run `git fetch origin master` and `git merge-base --is-ancestor <mergeCommit> origin/master` to fill `masterSha` and `ancestryVerified`. **This step is not assigned to the implementation Agent Run**, which has already completed, and it is not a file: it is the single durable handoff issue #98 consumes. A pull request closed unmerged or superseded gets an `implementation_merge_invalidated` activity instead, resetting the gate.

   `docs/reviews/goal-5a0-implementation-evidence.md` and the runbook's "Inbox 3a handoff" section carry the **procedure, the producers, and the payload schema** — not the post-merge values. They may retain stage-1 placeholders, they do not claim to contain the merge commit, and neither is rewritten after the merge; a document committed inside the implementation pull request cannot name that pull request's own merge commit. Issue #97 is closed as a consequence of the merge; closing it is not itself evidence.

**Verification:** Every reviewer checklist answer is backed by a file/command/test/result in the evidence document. The final invariant query returns zero rows. `git diff --check` is clean, the reviewed SHA matches the evidence SHA, and the disposition ledger contains every finding from every review round. The AgentOS task output for issue #97's implementation task carries the three stage-1 fields, each either a real value or its literal placeholder and none an invented or predicted pull-request number; the evidence document and the runbook carry the stage-2/stage-3 procedure, producers, and payload schema with no post-merge value asserted and no requirement to rewrite them after the merge; and the merge-closeout step's `implementation_merge_handoff` activity — written after this task is complete — is the only place the merge commit appears, validated by dry check 0 on retrieval and re-proved by `git merge-base --is-ancestor <mergeCommit> origin/master` after `git fetch origin master`. Dry check A's `gh pr view --json` field list is verified against `gh pr view --json` with no value on the tree's installed `gh` before the evidence packet is closed, and both `jq` classifiers are run against the four dry fixtures with their outcome tokens recorded.
...
2. When that pull request is merged, issue #98's first action is to run the three dependency-gate dry checks below — retrieve the durable handoff activity, confirm GitHub still agrees with it, confirm ancestry — and, on `SAFE_TO_IMPLEMENT`, rerun this plan's Step 1 revalidation against the merged tail, recording the merged Goal migration folder name and the merged state of every symbol in the ownership table.
...
| `ACTUAL_IMPL_HEAD` | the 40-hex head SHA the implementation run last pushed to that pull request | every push that moves the head | `not yet pushed` |

Stage 1 records no authorization and no merge commit. The implementation Agent Run completes before either exists, so it can never be the producer of either; assigning them to it is what made round 4's requirement impossible.

**Stage 2 — before the merge. Producer: the delegated exact-head authorizer — the independent operator holding the exact-head authorization role of `docs/governance/task-routing-v1.md` §"Human approval placement" — as an AgentOS TaskActivity on issue #97's implementation task.**

One activity per authorization, written *before* the merge, recording: the pull-request number and URL, the exact 40-hex **reviewed head**, the exact 40-hex **base** it was reviewed against, the required-check names with their observed conclusions at that head, and the review evidence consulted. This is the one-time delegated exact-head authorization the routing contract requires before a merge; head, base, required-check, or material-evidence drift invalidates it, and a re-authorization is a **new** activity. The latest non-invalidated activity governs. It authorizes the merge only; it is neither implementation authorization nor the handoff.

**Stage 3 — after the merge. Producer: the same delegated operator, at the merge-closeout step, as one AgentOS TaskActivity on issue #97's implementation task with the stable kind `implementation_merge_handoff`.**

This activity is **the single durable handoff consumed by issue #98**. Exactly one non-superseded instance may exist per implementation task. Its payload:

| Key | Value |
| --- | --- |
| `kind` | the literal `implementation_merge_handoff` |
| `issueNumber` | `97` |
| `implementationTaskId` | the exact AgentOS task ID of issue #97's implementation task |
| `prNumber` | `ACTUAL_IMPL_PR_NUMBER` as recorded in stage 1 |
| `prUrl` | `ACTUAL_IMPL_PR_URL` as recorded in stage 1 |
| `authorizedReviewedHead` | the exact 40-hex reviewed head from the governing stage-2 authorization |
| `mergeCommit` | the exact 40-hex merge commit that merge produced |
| `mergedAt` | the pull request's `mergedAt` timestamp, verbatim from `gh` |
| `masterSha` | the `origin/master` SHA observed at closeout, after `git fetch origin master` |
| `ancestryVerified` | `ANCESTOR`, from `git merge-base --is-ancestor <mergeCommit> origin/master` run freshly at closeout |

The merge-closeout step runs *after* the merge, so this is the first moment at which any record can hold the merge commit — and it is an activity on the task, not a file inside the merged tree, so nothing is self-referential. The merge may only proceed on an exact-head match: the head actually merged must equal the governing stage-2 `authorizedReviewedHead`, and `authorizedReviewedHead` in this payload is that same value, not a value re-derived after the fact.

**Invalidation and supersession.** If the pull request is closed without merging, or is superseded by another pull request, the delegated operator writes one activity of kind `implementation_merge_invalidated` naming the abandoned `prNumber` and the reason. That resets the gate: stage-1 fields return to their placeholders for the successor pull request, a fresh stage-2 authorization is required, and no earlier reviewed head, merge commit, or handoff is carried forward. A superseded handoff is never silently reused, and no field may hold a guessed or predicted pull-request number; `mergeCommit` may never hold an issue number, a branch name, or the reviewed head.

**What the documents hold.** `docs/reviews/goal-5a0-implementation-evidence.md` and the runbook's "Inbox 3a handoff" section define **the procedure and this payload schema**, name the producer of each stage, and may keep the stage-1 placeholders indefinitely. They do not claim to contain the merge commit, they are not evidence of a merge, and **they are not required to be rewritten after the merge**. Where they reference a post-merge value, they reference the `implementation_merge_handoff` activity that carries it.

### Dependency-gate dry checks

Any work stream that believes the Goal 5a0 dependency is discharged — issue #98 first, and any other consumer of this handoff — runs all three checks in order and records their raw output before touching code. All three are read-only. The only outcome tokens are `STOPPED_FOR_REROUTE` and `SAFE_TO_IMPLEMENT`; there is no partial credit and no third outcome.

**Dry check 0 — retrieve the durable handoff record.**

Read the AgentOS TaskActivity list of the exact `implementationTaskId` recorded for issue #97's implementation task, newest first, and select the latest `implementation_merge_handoff` activity that no later `implementation_merge_invalidated` activity names. `STOPPED_FOR_REROUTE` when any of:

- **missing** — no `implementation_merge_handoff` activity exists on that task;
- **duplicate** — more than one non-superseded `implementation_merge_handoff` exists on that task;
- **conflicting** — two handoff activities disagree on `prNumber`, `authorizedReviewedHead`, `mergeCommit`, or `mergedAt`;
- **superseded** — the selected handoff is named by a later `implementation_merge_invalidated`, or a later handoff records a different `prNumber`;
- **mismatched or malformed** — any key in the stage-3 table is absent, empty, or a placeholder; `mergeCommit` or `authorizedReviewedHead` fails `^[0-9a-f]{40}$`; `ancestryVerified` is not `ANCESTOR`; `issueNumber` is not `97`; or the activity is attached to any task other than the exact recorded `implementationTaskId`.

The retrieved payload is the **handoff record** that checks A and B compare against. Issue #97's own state is context, never authority: a closed issue with no valid handoff is `STOPPED_FOR_REROUTE`, because closing a work ticket is not a merge.

**Dry check A — does GitHub still agree with the handoff record?**

This check must be runnable with the `gh` that is actually installed. **`gh pr view --json` has no `merged` field** — round 4's field list made the command exit non-zero and the whole gate unrunnable. The supported fields used here are `number`, `state`, `mergedAt`, `mergeCommit`, `headRefOid`, and `url`; a merged pull request has `state: "MERGED"`, which is what replaces the missing boolean. Before changing this list, run `gh pr view --json` with no value and use only names it prints; before trusting the classifier, run it against the four fixtures below.

```sh
set -o pipefail
gh issue view 97 --json number,state,title,url                 > issue-97.json       || true  # context only
gh issue view 97 --json closedByPullRequestsReferences         > issue-97-links.json || true  # context only

if ! gh pr view "$HANDOFF_PR_NUMBER" \
      --json number,state,mergedAt,mergeCommit,headRefOid,url  > pr-view.json 2> pr-view.err; then
  echo STOPPED_FOR_REROUTE   # nonexistent pull request, unsupported field, auth/network or other CLI failure
  exit 0
fi
```

Classify the response — a JSON parse error, a non-object body, or a missing key is a stop, and `mergeCommit` is `null` on every unmerged pull request, so `.oid` is extracted null-safely rather than indexed directly:
...
' pr-view.json || echo STOPPED_FOR_REROUTE   # jq exit ≠ 0 means unparseable output: stop
```
...
jq -er --arg pr "$HANDOFF_PR_NUMBER" --arg url "$HANDOFF_PR_URL" \
       --arg head "$HANDOFF_AUTHORIZED_REVIEWED_HEAD" \
...
' pr-view.json || echo STOPPED_FOR_REROUTE
```

**Merged success is exactly this conjunction:** `state == "MERGED"`, `mergedAt` non-null, `mergeCommit.oid` matching `^[0-9a-f]{40}$`, and `number`/`url`/`headRefOid`/`mergeCommit.oid`/`mergedAt` all equal to the handoff record's `prNumber`/`prUrl`/`authorizedReviewedHead`/`mergeCommit`/`mergedAt`. A `headRefOid` that no longer equals `authorizedReviewedHead` is head drift: the merge did not happen at the authorized head, so it is a disagreement and a stop, not a warning. Everything else — nonexistent pull request, CLI failure, JSON parse or schema error, missing field, `state: "OPEN"`, `state: "CLOSED"` with a null `mergedAt`, or any field disagreement — is `STOPPED_FOR_REROUTE`.

**Dry fixtures.** Save these four as files and run both `jq` programs against them; the classifier is not trusted until all four produce the stated token. They depend on no unsupported field.

| Fixture | `pr-view.json` content (or CLI behaviour) | Required outcome |
| --- | --- | --- |
| absent | `gh` exits non-zero (`Could not resolve to a PullRequest`), leaving `pr-view.json` empty or non-JSON | `STOPPED_FOR_REROUTE` |
...
A fifth negative worth keeping: a `MERGED` fixture whose `mergeCommit` is `{}` must classify `STOPPED_FOR_REROUTE`, proving the null-safe extraction rejects a missing `oid` instead of throwing.

**Dry check B — is the recorded merge actually in `master`?**

```sh
git fetch origin master
git merge-base --is-ancestor "$HANDOFF_MERGE_COMMIT" origin/master && echo ANCESTOR || echo NOT_ANCESTOR
git rev-parse origin/master
```

Outcome `SAFE_TO_IMPLEMENT` only when dry check 0 returned one valid handoff record, dry check A returned `PR_MERGED_CANDIDATE` **and** `HANDOFF_AGREES`, and this check — run after a fresh `git fetch origin master` — prints `ANCESTOR`. `NOT_ANCESTOR`, a stale fetch, an unknown commit, or rewritten history is `STOPPED_FOR_REROUTE`. `SAFE_TO_IMPLEMENT` authorizes only Step 1's revalidation against the observed `origin/master` tip; it is not implementation authorization by itself, and it expires if `origin/master` moves before Step 1 completes — refetch and rerun check B.

All three checks record the retrieved handoff payload, the raw command output and exit statuses, the `origin/master` SHA observed, and the resulting outcome token. No other evidence of the dependency is accepted: not a green CI badge, not a closed issue, not a document that names a merge commit, not a branch that appears to contain the work.
...
| docs | the dependency identity — issues #97/#98 versus the actual implementation pull request | Goal 5a0 records the three-stage identity lifecycle and the three dry checks | Step 15.6 persists the stage-1 fields in the AgentOS task output and assigns stage 2 to the delegated exact-head authorizer and stage 3 (`implementation_merge_handoff`) to the delegated operator at merge closeout; Step 14.5 reproduces the schema, producers, and all three dry checks in the runbook as procedure, not as a record | issue #98 and PR #95 consume the durable handoff activity and the dry-check outcomes; neither may restate the dependency as "#97 merged", invent a pull-request number, treat a document as the merge record, or use a `gh --json` field the installed `gh` does not support |
...
Inbox 3a's migration timestamp must sort strictly after **the actual merged Goal tail** — the folder name as it exists on `master` at the `mergeCommit` carried by the valid `implementation_merge_handoff` activity, after the Goal 5a0 implementation pull request has merged and dry check B has printed `ANCESTOR` — not merely after the literal string `20260818000000`, and not after whatever is on any branch while issue #97 is still open. If the ordering fuse forced Goal 5a0 to a different folder name, that new name is the tail issue #98 must clear.
...
| Dependency expressed as the actual implementation pull request — issues #97/#98 never "merge", lifecycle-separated identity (stage-1 task output, stage-2 exact-head authorization activity, stage-3 `implementation_merge_handoff` activity written at merge closeout), no self-referential merge commit in any committed document, no invented PR number | 14, 15 |
| Executable dependency gate — three dry checks, only `gh`-supported JSON fields, null-safe `mergeCommit.oid`, explicit merged-success conjunction, every failure mode mapped to `STOPPED_FOR_REROUTE`, four dry fixtures | 14, 15 |
...
Inbox 3a (issue #98) stays dependency-held until the actual Goal 5a0 implementation pull request — recorded under "Implementation identity and durable handoff record", never issue #97 itself — has merged, dry check 0 retrieves exactly one valid `implementation_merge_handoff` activity on issue #97's implementation task, dry check A finds GitHub still agreeing with it field for field, dry check B prints `ANCESTOR` against a freshly fetched `origin/master`, and Step 1's revalidation is rerun on that tail. Goal 5a0 discharges that boundary with the Step 9 fence and seam, the three-stage identity lifecycle with its named producers, and the written handoff; it does not implement Inbox 3a, and it does not write the post-merge record itself — the delegated operator does, at merge closeout.

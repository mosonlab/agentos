...
Product Contract: Goal 5a0 v1.2

Contract-version note (rounds 6 and 7, 2026-08-17): the contract moved from v1.0 to v1.1, and then from v1.1 to v1.2, to carry the merge-authorization and dependency-handoff **evidence and authority mechanics** recorded in `docs/plans/goal-5a0-idempotent-execution-kernel-plan.md` §"Product Contract version decisions". v1.1 added typed, schema-versioned handoff records; a fail-closed server-bound authorization half; and named implementation dependency D1. v1.2 pins the merge method as a gate precondition and captures it as authorized evidence; makes the evidence digest's preimage identical to the tuple the handoff copies; replaces the formal `APPROVED` review — which GitHub makes unreachable for this repository's single authorized operator, since it forbids a pull request's author from approving it — with an allowlisted, unedited authorization **comment** whose canonical marker binds head, base, and evidence digest; adds an authoritative required-check policy with exact set equality and a fail-closed unreadable-source stop; restricts the external locator to an attributable comment and removes every credential destination from GitHub-controlled text; replaces lexical `(createdAt, id)` supersession with reference-graph liveness and names dependency **D2**; and requires every terminal stop to name a recovery path. D1 and D2 are both outside this contract's implementation scope. **No invariant, data contract, transition, acceptance proof, reviewer checklist item, or scope boundary in this specification is changed by either bump**; every section below is carried forward verbatim from v1.0.

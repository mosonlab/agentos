...
// --- instantiation-facing resolution -----------------------------------------

const staffingLine = /^Staffing: (.+)$/mu;
...
export const parseStaffingProfileName = (description: string | undefined): string | null => {
  const value = description?.match(staffingLine)?.[1];
  if (!value) return null;
  const name = value.trim();
  return name.length > 0 && name.length <= 200 && name === value ? name : null;
};

/**
 * The first line claiming to be a staffing selection that does not match the
 * grammar. Same discipline as `Route:`: a near-miss must refuse rather than
 * silently staff the chain from the default profile.
 */
...
/**
 * What instantiation needs from a profile, resolved under the locks it already
 * holds.
 *
 * Call it after `lockTemplateRow` inside the serializable callback. `agentIds`
 * joins the caller's `lockAgentRows` set; `assigneeFor` and `includeOptional`
 * answer per step, and `null`/`undefined` mean "no opinion", so the caller
 * keeps the canonical binding. Resolution order stays the caller's:
 * `stepOverrides[stepIndex]` beats this, and this beats the canonical step.
 */
export type StaffingResolution = {
  profile: { id: string; name: string } | null;
  agentIds: string[];
  /** The profile's assignee for one step, or null when it has no opinion. */
  assigneeFor(step: { outputKind: string }): string | null;
  /** Whether an optional step is kept. `true` when the profile says nothing. */
  includeOptional(step: { outputKind: string }): boolean;
};

const EMPTY_RESOLUTION: StaffingResolution = {
  profile: null,
  agentIds: [],
  assigneeFor: () => null,
...
export const resolveStaffingForTemplate = async (
  tx: Tx,
  input: {
    projectId: string;
    taskTemplateId: string;
    /** Explicit selection; refuses when it is not a profile of this template. */
    staffingProfileId?: string | undefined;
    /** `Staffing: <name>` selection; refuses when the template has no such profile. */
    staffingProfileName?: string | undefined;
  },
): Promise<StaffingResolution> => {
  const where = input.staffingProfileId !== undefined
    ? { id: input.staffingProfileId, taskTemplateId: input.taskTemplateId }
    : input.staffingProfileName !== undefined
      ? { taskTemplateId: input.taskTemplateId, name: input.staffingProfileName }
      : { taskTemplateId: input.taskTemplateId, isDefault: true };
  const profile = await tx.staffingProfile.findFirst({ where, select: profileSelect });
  if (!profile) {
    if (input.staffingProfileId !== undefined) {
      throw refuse(
        "staffing_profile_not_found",
        `Staffing profile ${input.staffingProfileId} is not a profile of this template`,
      );
    }
    if (input.staffingProfileName !== undefined) {
      throw refuse(
        "staffing_profile_not_found",
        `Template has no staffing profile named ${input.staffingProfileName}`,
      );
    }
    // No default profile: the template is staffed exactly as it was authored.
    return EMPTY_RESOLUTION;
  }
  const byKind = new Map(profile.entries.map((entry) => [entry.outputKind, entry]));
  return {
    profile: { id: profile.id, name: profile.name },
    agentIds: [...new Set(profile.entries.flatMap((entry) => (
      entry.assigneeAgentId === null ? [] : [entry.assigneeAgentId]
    )))].sort(),
    assigneeFor: (step) => byKind.get(step.outputKind)?.assigneeAgentId ?? null,
    includeOptional: (step) => byKind.get(step.outputKind)?.include ?? true,
  };
};

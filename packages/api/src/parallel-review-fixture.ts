...
export type SpecificationRead = { repository: string; path: string; commitSha: string };

export const createParallelReviewHarness = ({
  getDb,
  ...
  const agents = await getDb().agent.findMany({ where: { projectId }, select: { id: true } });
  ...
  const project = await getDb().project.findUniqueOrThrow({ where: { slug: "agentos-example" } });
 ...
 * implementation-to-parallel-review boundary through the runner API. */
  ...
  await getDb().task.updateMany({ where: { id: { in: priorIds } }, data: { status: TaskStatus.DONE } });
  const specificationTask = byIndex.get(1)!;
  await getDb().taskStepOutput.create({ data: {
  ...
  const queuedRunsFor = (taskIds: string[]) => getDb().run.findMany({

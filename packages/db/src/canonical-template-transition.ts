...
/**
 * The two graphs that existed immediately before the adjudication node was
 * removed. These are intentionally a closed contract: a row with the canonical
 * name is either one of these exact graphs or the current source graph. It is
 * never guessed at and it is never linearized as a fallback.
 */
const PRE_ADJUDICATION_TEMPLATE_SHAPES = {
...
/**
 * The rename target is minted per row: the fixed `-legacy-v1` identity is
 * already taken by the graph that preceded the layered review split, so an
 * adjudication-era row needs an identity of its own to roll over onto.
 */
export const legacyAdjudicationTemplateName = (templateName: string, templateId: string): string =>
  `${templateName}-legacy-pre-adjudication-${templateId}`;
...
const legacyShapeFor = (templateName: string): readonly (readonly [
  ...
  if (templateName === "direct-engineer-workflow" || templateName === "compound-engineer-workflow") {
    return PRE_ADJUDICATION_TEMPLATE_SHAPES[templateName];
  }
  return null;
};

/**
 * Return a named refusal reason when a canonical row is neither the exact
 * adjudication-era graph nor the exact current graph. The caller runs this for every row before

...
| 6 | 路由默认（历史，模型默认已被 #22 取代） | 原始定案为规划/spec/评审类走 claude、实现类走 codex、librarian 走 pi/Luna；该默认不再用于创建 Run。逐 agent `model`+`runner` 配置机制仍保留，当前值以 #22 为准。 | 保留早期决策历史；2026-08-17 的 canonical 默认见 #22。 |
...
| 11 | Agent 名册（历史，active roster 已被 #22 与 2026-08-17 convergence 取代） | 原始十角色名册保留为决策历史；`scope-guardian`、`coherence` 已删除，`plan-reviser` 与 `frontend-dev` 已进入 canonical 配置，`feasibility`/`code-reviewer` 仅在 live/queued reference 清零前保留兼容。当前角色和默认以 `agents/roles/*.md`、`packages/db/prisma/agent-contract.ts` 与 #22 为准。 | 防止旧名册被误当成当前 seed/routing authority。 |
...
| 19 | Agent 提示词管线（历史，角色数与评审拓扑已被 2026-08-17 convergence 取代） | 最初对十角色执行 writing-for-agents、互盲双写与交叉评审；该流程仅解释早期 prompt 来源，不再定义 active roster、review delegation 或模型路由。当前角色/模型见 `agents/roles/*.md`、`packages/db/prisma/agent-contract.ts` 与 #22，当前评审职责见 `agents/roles/review-coordinator.md`。 | 保留 prompt 形成史；防止旧数量和旧多-agent 拓扑被当成新任务模板。 |
| 20 | agentrunner 时点 | 裁定#4 保留，执行时点=开始无人值守跑真实仓库之前；v2 Docker 落地后退役 | 过渡方案按过渡成本衡量 |
| 21 | Agent 模型与提示词解耦（Leo 2026-08-17 改裁，默认模型部分已被 #22 取代） | 角色标题与任务提示词不得写模型或厂商名，模型、runner、推理档位只在 agent 配置中选择；review prompt 统一使用 `REVIEW INDEPENDENCE` 证据规则 | 额度变化不应迫使重写任务语义；把厂商写进 prompt 会在切模后产生事实错误并污染评审判断 |
| 22 | Canonical Agent 模型默认（Leo 2026-08-17 夜间最终改裁） | `spec` 与 `plan` = CLAUDE / `claude-fable-5:medium`；`plan-reviser`、`implementation-plan-executioner`、`frontend-dev`、`senior-dev` = CLAUDE / `claude-opus-5:high`；`review-coordinator` 保持 CODEX / `gpt-5.6-sol:high`；`librarian` 保持 CODEX / `gpt-5.6-luna:high`。只影响配置变更后创建的 Run，既有 Run 快照不可变。 | Leo 明确批准 Specification Writer 从 canonical high 下调到 medium；Planner 同用 Fable Medium。Plan Reviser 与三个实现角色使用 Opus High。provider/model 只在配置层变更，role prompt 继续解耦。Fable 恢复为正式 web catalog 模型。 |

...
test("the chain marker reads done/total, step, status", () => {
  assert.equal(
    chainMarker({ chainId: "c1", done: 4, total: 9, activeStepName: "Implementation", activeStatus: "doing" }),
...
test("a finished chain still reads as n/n rather than collapsing", () => {
  assert.equal(
    chainMarker({ chainId: "c1", done: 3, total: 3, activeStepName: "Review", activeStatus: "done" }),
    "3/3 · Review · done",
  );
});

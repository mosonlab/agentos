...
`codex exec --skip-git-repo-check -C <worktree> -m "$AGENTOS_SUBORDINATE_CODEX_MODEL" -c "model_reasoning_effort=\"$AGENTOS_SUBORDINATE_CODEX_REASONING_EFFORT\"" -c "service_tier=\"$AGENTOS_SUBORDINATE_CODEX_SERVICE_TIER\"" "<slice prompt>" </dev/null > <slice-id>.log 2>&1 &`

The three `AGENTOS_SUBORDINATE_CODEX_*` values are platform-pinned Run snapshot authority for ordinary slices. The slice prompt carries the slice file's full text and the path to spec.md. A slice whose frontmatter flags risk runs instead at `-m gpt-5.6-sol -c model_reasoning_effort=high -c service_tier="default"`. Pass `-m`, the effort config, and `service_tier` explicitly on every launch and again on every `codex exec resume` — a resume without them silently loses the profile. Record each subprocess's session id, log path, and exit status in the activity log. Keep your own session's default sandbox: worktrees need a writable `.git`.

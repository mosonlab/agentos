...
`codex exec --skip-git-repo-check -C <worktree> -m "$AGENTOS_ORDINARY_CODEX_SUBPROCESS_MODEL" -c "model_reasoning_effort=\"$AGENTOS_ORDINARY_CODEX_SUBPROCESS_REASONING_EFFORT\"" -c "service_tier=\"$AGENTOS_ORDINARY_CODEX_SUBPROCESS_SERVICE_TIER\"" "<slice prompt>" </dev/null > <slice-id>.log 2>&1 &`

The `AGENTOS_ORDINARY_CODEX_SUBPROCESS_*` values are platform-pinned Run snapshot authority for ordinary slices. The slice prompt carries the slice file's full text and the path to spec.md. A slice whose frontmatter flags risk uses the corresponding `AGENTOS_ELEVATED_CODEX_SUBPROCESS_*` model, reasoning effort, and service tier instead. Pass `-m`, the effort config, and `service_tier` explicitly on every launch and again on every `codex exec resume` — a resume without them silently loses the profile. Record each subprocess's session id, log path, and exit status in the activity log. Keep your own session's default sandbox: worktrees need a writable `.git`.
